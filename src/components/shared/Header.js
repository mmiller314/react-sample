...
import logo from '../../content/images/scientist.png';
import chicken from '../../content/images/chicken.png';
...
class Header extends React.Component {
  ...
  render() {
    return (
      <header className="navbar navbar-expand navbar-dark flex-column flex-md-row bd-navbar shadow p-3 mb-5">
        <div className="container">
          <Link className="navbar-brand mr-0 mr-md-2" to="/" aria-label="algorithms">
            <img src={chicken} width="64" height="64" />
          </Link>
          <div className="navbar-nav-scroll">
            <ul className="navbar-nav bd-navbar-nav flex-row">
              {this.props.links.map(l => (
                <li key={l.id} className="nav-item">
                  <NavLink className="nav-link" to={l.link} activeClassName="active" exact>{l.name}</NavLink>
                </li>
              ))}
              {this.props.loading && <li className="nav-item">
          ...
          <ul className="navbar-nav flex-row ml-md-auto d-none d-md-flex">
            ...
            <li className="nav-item">
              <a className="nav-link p-2" href="javascript:void(0)">
